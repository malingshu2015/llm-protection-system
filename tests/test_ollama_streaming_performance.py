...
from src.web.api import app, stream_ollama_response, _response_cache
...
def test_ollama_chat_stream_performance(client):
    ...
    with patch("src.web.api.stream_ollama_response", return_value=mock_stream_response()):
        ...
        # 验证缓存中有数据
        cache_key = hash(f"llama2_{str([{'role': 'user', 'content': 'Hello'}])}_{{}}") 
        assert cache_key in _response_cache
        assert 'chunks' in _response_cache[cache_key]
...
def test_ollama_chat_non_stream_performance(client):
    ...
    with patch("subprocess.run", return_value=mock_result):
        ...
        cache_key = hash(f"llama2_{str([{'role': 'user', 'content': 'Hello'}])}_{{}}") 
        assert cache_key in _response_cache
        assert 'full_response' in _response_cache[cache_key]
