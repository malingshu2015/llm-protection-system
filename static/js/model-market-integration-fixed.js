...
class ModelMarketIntegration {
    ...
    updateMarketStats(stats) {
        ...
        // 安全处理统计数据，确保数据存在
        const totalModels = stats?.total_models || stats?.total_models_count || 0;
        const totalDownloads = stats?.total_downloads || stats?.total_downloads_count || 0;
        const totalLikes = stats?.total_likes || stats?.total_likes_count || 0;
        const newThisWeek = stats?.new_this_week || stats?.new_models_this_week || 0;

        updateElement('total-models', totalModels.toLocaleString());
        updateElement('total-downloads', totalDownloads.toLocaleString());
        updateElement('total-likes', totalLikes.toLocaleString());
        updateElement('new-this-week', newThisWeek.toLocaleString());
    }
